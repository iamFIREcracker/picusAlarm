/** \file WFlib.c
 *  \brief Function to manage to Wifi module and to change at runtime the network settings.
 */
...
@{
<H3>Introduction</H3>
The <B>WiFi library </B>contains all the functions to manage the 
WiFi stack. It's possible to connect or disconnect from a network, change the settings, and save them inside the falsh memory of the device, to recall at the startup.

<H3>Connection profiles</H3>
A <VAR>connection profile</VAR> contains all the information to connect to or to create a WiFi network. The Flyport module has two possible profile to use:
<UL>
...
/**
 * WFConnect - Verifies how many bytes can be read from the specified TCP socket.
...
/**
 * WFCustomSave - When the Flyport is powered down, all the changes on the WF_CUSTOM profile will be lost, because are stored in RAM. To prevent the loosing of all the data, you can use the 
 command WFCustomSave. It saves all the network parameters for the WF_CUSTOM connection profile. No need to set anything, the data will be saved in a reserved part of the flash memory, so will
 ...
 * \param paramstring - value of the parameter.
 * \return None.
 * \paragraph usage Usage
 <B>Sintax of the command to set the parameters: </B>
 <UL>
	<LI><B>IP address of the device:</B> WFSetParam(MY_IP_ADDR , string with the IP address, for example "192.168.1.100")</LI><BR>
	<LI><B>Primary DNS server:</B> WFSetParam(PRIMARY_DNS , string with the IP address, for example "192.168.1.1")</LI><BR>
	<LI><B>Secondary DNS server:</B> WFSetParam(SECONDARY_DNS , string with the IP address, for example "192.168.1.1")</LI><BR>
	<LI><B>Default gateway:</B> WFSetParam(MY_GATEWAY , string with the IP address, for example "192.168.1.1")</LI><BR>
	<LI><B>Subnet mask:</B> WFSetParam(SUBNET_MASK , string with the subnet mask, for example "255.255.255.0")</LI><BR>
	<LI><B>Netbios name:</B> WFSetParam(NETBIOS_NAME , string with the Netbios name, for example "Flyport")</LI><BR>
	<LI><B>SSID:</B> WFSetParam(SSID_NAME , string with the SSID name, for example "FlyportNet")</LI><BR>
	<LI><B>DHCP client enabled or not:</B> WFSetParam(DHCP_ENABLE , ENABLED or DISABLED)</LI><BR>
	<LI><B>Network type (adhoc or infrastructure):</B> WFSetParam(NETWORKTYPE , ADHOC or INFRASTRUCTURE)</LI> <BR>
 </ul>
 */
void WFSetParam(int paramtoset , char* paramstring)
{
	...
	case MY_IP_ADDR:
		StringToIPAddress((BYTE*) paramstring, &(WFConf[0].DefaultIPAddr));
		StringToIPAddress((BYTE*) paramstring, &(WFConf[0].MyIPAddr));
		break;
	//	Changing primary DNS server
	case PRIMARY_DNS:
		StringToIPAddress((BYTE*) paramstring, &(WFConf[0].PrimaryDNSServer));
		break;
	//	Changing secondary DNS server
	case SECONDARY_DNS:
		StringToIPAddress((BYTE*) paramstring, &(WFConf[0].SecondaryDNSServer));	
		break;		
	//	Changing Gateway
	case MY_GATEWAY:
		StringToIPAddress((BYTE*) paramstring, &(WFConf[0].MyGateway));
		break;
	//	Subnet mask
	case SUBNET_MASK:
		StringToIPAddress((BYTE*) paramstring, &(WFConf[0].MyMask));	
		StringToIPAddress((BYTE*) paramstring, &(WFConf[0].DefaultMask));
		break;
	//	Netbios name
	case NETBIOS_NAME:
		strcpy( (char *) WFConf[0].NetBIOSName,paramstring);
		break;
	//	SSID name
	case SSID_NAME:
		strcpy( (char *) WFConf[0].MySSID,paramstring);
		WFConf[0].SsidLength = strlen( (char *) WFConf[0].MySSID);
		break;	
	//	DHCP enabling
	case DHCP_ENABLE:
		if (!strcmp(paramstring,"On"))
			WFConf[0].Flags.bIsDHCPEnabled = TRUE;
		else if (!strcmp(paramstring,"Off"))
			WFConf[0].Flags.bIsDHCPEnabled = FALSE;
		break;
	case NETWORK_TYPE:
		if (!strcmp(paramstring,"infra"))
			WFConf[0].networkType = WF_INFRASTRUCTURE;
		else if (!strcmp(paramstring,"adhoc"))
			WFConf[0].networkType = WF_ADHOC;
	}
}
...
{
	WFConf[0].SecurityMode = mode;
	WFConf[0].SecurityKeyLength = (BYTE) keylen;
	WFConf[0].WepKeyIndex = keyind;
	int isec=0;

	for (isec=0 ; isec<keylen ; isec++)
	{
		WFConf[0].SecurityKey[isec] = *(keypass + isec);
	}
}

/**
 * WFHibernate - Enables Hibernate mode on the MRF24WB0M, which effectively turns off the device for maximum power savings. At the moment to exit from hybernate state a Reset() is nedded.
 * \param None
 * \return None
 */
void WFHibernate()
