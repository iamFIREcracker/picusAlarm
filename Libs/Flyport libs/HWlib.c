...
void UARTInit(int port,long int baud)
{
	...
	if (baudcalc>baud)
		err=((baudcalc-baud)*100ul)/baud;
	else
		err=((baud-baudcalc)*100ul)/baud;
	if (err<2)
	{
		*UMODEs[port] = 0;
		*UBRGs[port] = brg;
...
void UARTWrite(int port, char *buffer)
{
	...
	port = port-1;
	int pdsel;
    // transmits till NULL character is encountered 
...
//	PPS configuration
		...
		RPINR19bits.U2RXR = 17;					// Assign RP17 to U2RX (input)
		RPOR8bits.RP16R = 5;					// Assign RP16 to U2TX (output)	
